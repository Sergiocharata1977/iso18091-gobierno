...
export const POST = withAuth(
  async (request, context, auth) => {
    try {
      ...
      const installed = await PluginLifecycleService.disablePlugin({
      ...
      console.error('[admin/plugins/disable][POST] Error:', error);
      return NextResponse.json(
        { success: false, error: 'No se pudo deshabilitar el plugin' },
        { status: 500 }
      );
    }
  },

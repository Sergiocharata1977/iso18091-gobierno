...
import type { Offboarding } from '@/types/rrhh-plugins';
...
export const GET = withAuth(
  async (_request, context, auth) => {
    try {
      ...
      const doc = await db.collection('rrhh_offboarding').doc(id).get();
    ...
    try {
      ...
      if (updates.tareas) {
        ...
        const allDone = updates.tareas.every(
          (t: Offboarding['tareas'][0]) => t.estado === 'completada' || t.estado === 'omitida'
        );
        if (allDone) updates.estado = 'completado';
      }

      await db.collection('rrhh_offboarding').doc(id).update({ ...updates, updated_at: new Date().toISOString() });
      return NextResponse.json({ id, ...data, ...updates });
    } catch {
      return NextResponse.json({ error: 'Error al actualizar offboarding' }, { status: 500 });
    }
  },

...
export const GET = withAuth(
  async (_request, context, auth) => {
    try {
      ...
      const doc = await db.collection('rrhh_candidatos').doc(id).get();
...
export const PUT = withAuth(
  async (request, context, auth) => {
    try {
      ...
      await db.collection('rrhh_candidatos').doc(id).update({ ...updates, updated_at: new Date().toISOString() });
...
export const DELETE = withAuth(
  async (_request, context, auth) => {
    try {
      ...
      await db.collection('rrhh_candidatos').doc(id).delete();

...
export const PUT = withAuth(
  async (request, context, auth) => {
    ...
    const doc = await db.collection('rrhh_planificacion').doc(id).get();
    ...
    await db.collection('rrhh_planificacion').doc(id).update({ ...updates, updated_at: new Date().toISOString() });
...
export const DELETE = withAuth(
  async (_request, context, auth) => {
    ...
    if (auth.role !== 'super_admin' && data.organization_id !== auth.organizationId)
      return NextResponse.json({ error: 'Acceso denegado' }, { status: 403 });
    await db.collection('rrhh_planificacion').doc(id).delete();

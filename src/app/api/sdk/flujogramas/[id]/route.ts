...
export const GET = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      if (!hasResourceAccess(flujograma, auth)) {
      ...
      const id = await resolveId(context);
      console.error(`Error in GET /api/sdk/flujogramas/${id}:`, error);
      return NextResponse.json(
        {
...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      const id = await resolveId(context);
      console.error(`Error in PUT /api/sdk/flujogramas/${id}:`, error);
      return NextResponse.json(
        {
...
export const DELETE = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      const id = await resolveId(context);
      console.error(`Error in DELETE /api/sdk/flujogramas/${id}:`, error);
      return NextResponse.json(
        {

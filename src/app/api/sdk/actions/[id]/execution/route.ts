...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      await service.updateExecution(id, validatedData, auth.uid);

      return NextResponse.json({
        message: 'Ejecuci�n de acci�n actualizada exitosamente',
        id,
        status: 'in_progress',
      });
    } catch (error) {
      const id = await resolveId(context);
      console.error(`Error in PUT /api/sdk/actions/${id}/execution:`, error);
      ...
      return NextResponse.json(
        {
          error: 'Error al actualizar ejecuci�n de acci�n',

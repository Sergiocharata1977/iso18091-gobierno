...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      await service.updateImmediateActionExecution(id, validatedData, auth.uid);

      return NextResponse.json({
        message: 'Ejecuci�n de acci�n inmediata actualizada exitosamente',
        id,
      ...
      const id = await resolveId(context);
      console.error(
        `Error in PUT /api/sdk/findings/${id}/immediate-action-execution:`,
        error
      );
      ...
      return NextResponse.json(
        {
          error: 'Error al actualizar ejecuci�n de acci�n inmediata',

...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      await service.updateRootCauseAnalysis(id, validatedData, auth.uid);

      return NextResponse.json({
        message: 'An�lisis de causa ra�z actualizado exitosamente',
        id,
      ...
      const id = await resolveId(context);
      console.error(
        `Error in PUT /api/sdk/findings/${id}/root-cause-analysis:`,
        error
      );
      ...
      return NextResponse.json(
        {
          error: 'Error al actualizar an�lisis de causa ra�z',

...
export const GET = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      if (!hasResourceAccess(reunion, auth)) {
      ...
      const id = await resolveId(context);
      console.error(`Error in GET /api/sdk/reuniones/${id}:`, error);
      return NextResponse.json(
        {
...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      return NextResponse.json(
        { message: 'Reunion actualizada exitosamente', id },
        { status: 200 }
      );
    } catch (error) {
      const id = await resolveId(context);
      console.error(`Error in PUT /api/sdk/reuniones/${id}:`, error);
      return NextResponse.json(
        {
...
export const DELETE = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      return NextResponse.json(
        { message: 'Reunion eliminada exitosamente', id },
        { status: 200 }
      );
    } catch (error) {
      const id = await resolveId(context);
      console.error(`Error in DELETE /api/sdk/reuniones/${id}:`, error);
      return NextResponse.json(
        {

...
export const GET = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      if (!id) {
        return NextResponse.json(
          { error: 'ID de politica requerido' },
          { status: 400 }
        );
      }
      ...
      if (!hasResourceAccess(policy, auth)) {
      ...
      const id = await resolveId(context);
      console.error(`Error in GET /api/sdk/policies/${id}:`, error);
      return NextResponse.json(
        {
...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      return NextResponse.json(
        { message: 'Politica actualizada exitosamente', id },
        { status: 200 }
      );
    } catch (error) {
      const id = await resolveId(context);
      console.error(`Error in PUT /api/sdk/policies/${id}:`, error);
      return NextResponse.json(
        {
...
export const DELETE = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      return NextResponse.json(
        { message: 'Politica eliminada exitosamente', id },
        { status: 200 }
      );
    } catch (error) {
      const id = await resolveId(context);
      console.error(`Error in DELETE /api/sdk/policies/${id}:`, error);
      return NextResponse.json(
        {

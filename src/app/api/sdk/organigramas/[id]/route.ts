...
export const GET = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      if (!hasResourceAccess(organigrama, auth)) {
      ...
      const id = await resolveId(context);
      console.error(`Error in GET /api/sdk/organigramas/${id}:`, error);
      return NextResponse.json(
        {
...
export const PUT = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      const id = await resolveId(context);
      console.error(`Error in PUT /api/sdk/organigramas/${id}:`, error);
      return NextResponse.json(
        {
...
export const DELETE = withAuth(
  async (request: NextRequest, context, auth: AuthContext) => {
    try {
      ...
      const id = await resolveId(context);
      console.error(`Error in DELETE /api/sdk/organigramas/${id}:`, error);
      return NextResponse.json(
        {

...
const statusSchema = z.object({
  status: z.enum(['abierta', 'pendiente_respuesta', 'en_gestion', 'resuelta', 'archivada', 'spam']),
});
...
export const PATCH = withAuth(
  async (request: NextRequest, context: RouteContext, auth) => {
    try {
      ...
      // When resolved or archived, record the resolved_at timestamp
      if (body.status === 'resuelta' || body.status === 'archivada') {
      ...
      console.error('[whatsapp/conversations/[id]/status][PATCH]', error);
      return NextResponse.json(
        { success: false, error: 'No se pudo cambiar el estado de la conversación' },
        { status: 500 }
      );
    }
  },

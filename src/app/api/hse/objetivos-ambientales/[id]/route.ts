...
const patchObjetivoBody = z.object({
  descripcion: z.string().min(5).optional(),
  meta: z.string().min(2).optional(),
  indicador: z.string().min(2).optional(),
  fecha_limite: z.string().optional(),
  responsable_uid: z.string().optional(),
  avance_porcentaje: z.number().min(0).max(100).optional(),
...
export const GET = withAuth(
  async (request: NextRequest, context: RouteContext, auth) => {
    try {
      ...
      if (!doc.exists) {
        return NextResponse.json(
          { success: false, error: 'Objetivo ambiental no encontrado' },
          { status: 404 }
        );
      }
      ...
      console.error('[hse/objetivos-ambientales/[id]][GET]', error);
      return NextResponse.json(
        { success: false, error: 'No se pudo obtener el objetivo ambiental' },
        { status: 500 }
      );
    }
  },
...
export const PATCH = withAuth(
  async (request: NextRequest, context: RouteContext, auth) => {
    try {
      ...
      const body = patchObjetivoBody.parse(await request.json());
      ...
      console.error('[hse/objetivos-ambientales/[id]][PATCH]', error);
      return NextResponse.json(
        { success: false, error: 'No se pudo actualizar el objetivo ambiental' },
        { status: 500 }
      );
    }
  },
...
export const DELETE = withAuth(
  async (request: NextRequest, context: RouteContext, auth) => {
    try {
      ...
      console.error('[hse/objetivos-ambientales/[id]][DELETE]', error);
      return NextResponse.json(
        { success: false, error: 'No se pudo eliminar el objetivo ambiental' },
        { status: 500 }
      );
    }
  },

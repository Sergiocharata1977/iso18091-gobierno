...
const FLOW = 'servicio';
const FLOW_LABEL = 'Servicios Tecnicos';
const FLOW_PLURAL = 'solicitudes de servicios';
const FLOW_BADGE =
  'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300';
const FLOW_EMPTY = 'Sin solicitudes de servicios tecnicos';
const FLOW_DESCRIPTION =
  'Tablero operativo para coordinar diagnosticos y trabajos de servicio tecnico.';
  ...
  return (
    <div className="grid gap-2 text-sm">
      ...
      {!!payload.descripcion_problema && (
        <div>
          <span className="font-medium text-slate-900">Problema tecnico:</span>
  ...
  return (
    <button
      ...
      className={`w-full rounded-2xl border p-4 text-left transition-all hover:shadow-md cursor-grab active:cursor-grabbing ${
        dragging ? 'opacity-40 ring-2 ring-amber-300' : ''
      } ${
        selected
  ...
  if (!solicitud) {
    return (
      <div className="flex h-full min-h-[320px] items-center justify-center text-center text-sm text-slate-500">
        Selecciona una solicitud de servicio para ver el detalle y editar su
        estado.
      </div>
    );
  }
...
export default function SolicitudesServiciosPage() {
  ...
  const loadSolicitudes = async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams({ tipo: FLOW });
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-slate-600">
                  Diagnosticos y reparaciones de maquinaria agricola.
                </p>
              </div>
            </div>
